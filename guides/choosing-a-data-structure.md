...
Sketch includes 8 data structures. This guide helps you pick the right one for your problem.

## Decision Tree

**"I need to check if an item is in a set"**

- Can items be removed? -> `Sketch.CuckooFilter`
- No deletion needed? -> `Sketch.BloomFilter` (simpler, slightly more space-efficient)

**"I need to count how often items appear"**

- Track all items' frequencies? -> `Sketch.CountMinSketch`
- Only need the top K most frequent? -> `Sketch.TopK`
...
|---|---|---|---|---|---|---|
| `Sketch.BloomFilter` | O(n) bits, tuned by capacity and FPP | False positive rate configurable (e.g. 1%, 0.1%); no false negatives | Yes (bitwise OR) | Yes (`to_binary` / `from_binary`) | No | Pre-filtering expensive lookups, deduplication |
| `Sketch.CuckooFilter` | ~12 bits per element (8-bit fingerprints, 4 slots/bucket) | False positive rate ~3% with 8-bit fingerprints; no false negatives | No | Yes (`to_binary` / `from_binary`) | Yes | Set membership when you also need to remove items |
| `Sketch.CountMinSketch` | `width x depth` counters, fixed by epsilon and delta | Overestimates by at most epsilon * N with probability 1 - delta; never undercounts | Yes (element-wise addition) | Yes (`to_binary` / `from_binary`) | No | Frequency estimation for arbitrary stream elements |
| `Sketch.TopK` | CMS + O(k) candidate map | Inherits CMS error bounds; heavy hitters are tracked accurately | Yes (CMS merge + candidate reunion) | No | No | Finding the most frequent items in a data stream |
| `Sketch.HyperLogLog` | 2^p bytes (e.g. 16 KiB at p=14) | Standard error ~1.04 / sqrt(2^p) | Yes (element-wise max) | Yes (`to_binary` / `from_binary`) | No | Counting unique visitors, distinct IPs, unique events |
| `Sketch.TDigest` | O(delta) centroids | High accuracy at tails (p99, p99.9); bounded by delta parameter | Yes (centroid merge + recompress) | Yes (`to_binary` / `from_binary`) | No | Latency percentiles, SLA monitoring, distribution summary |
| `Sketch.Reservoir` | O(k) elements | Exact uniform sample of k items from a stream of any length | Yes (weighted random selection) | No | No | Random sampling from unbounded streams |
| `Sketch.MinHash` | O(num_hashes) integers per signature | Standard error ~1 / sqrt(num_hashes) on Jaccard similarity | Yes (element-wise min on signatures) | No | No | Near-duplicate detection, set similarity estimation |

## "Why not just use..."

### MapSet / Map

Exact collections give you perfect answers, but they store every element. A `MapSet` holding 100 million strings will consume gigabytes of memory. A `Sketch.BloomFilter` configured for the same capacity at 1% false-positive rate uses roughly 114 MB of bits -- and a `Sketch.HyperLogLog` at precision 14 counts those 100 million distinct items in just 16 KiB.

**Use Sketch when**: your data volume makes O(n) memory impractical, or when an approximate answer is acceptable to avoid a database round-trip.
...
ETS tables are mutable, node-local, and still O(n) in memory. They cannot be merged across nodes in a distributed system without writing custom synchronization logic. Sketch structures are immutable, safely shareable across processes, and every one supports a `merge/2` operation for combining partial results from different nodes or time windows.

**Use Sketch when**: you need to aggregate across distributed nodes, want immutable data you can pass through message-passing pipelines, or need sub-linear memory.
...
Redis provides a built-in `PFADD` / `PFCOUNT` / `PFMERGE` for HyperLogLog, but it requires a Redis dependency, network round-trips, and operational overhead. Sketch is pure Elixir, runs in-process with zero latency, and gives you 7 additional data structures beyond cardinality estimation. Sketch structures can be serialized and stored in Redis, Postgres, or anywhere else if you need persistence.

**Use Sketch when**: you want to avoid an external dependency, need in-process speed, or need structures beyond HyperLogLog.
...
These BEAM primitives provide fast mutable integer arrays, but they are fixed-size -- you must know the number of slots (keys) in advance. They cannot dynamically handle arbitrary keys from a stream. A `Sketch.CountMinSketch` maps any term to a fixed-size table using hash functions, giving you frequency estimation for an unbounded key space in bounded memory.

**Use Sketch when**: the set of keys is unbounded or unknown in advance, or when you need mergeability and serialization.
...
```elixir
# 1% FPP, good default
bf = Sketch.BloomFilter.new(100_000, 0.01)

# 0.1% FPP, tighter but uses ~50% more memory
bf = Sketch.BloomFilter.new(100_000, 0.001)
```
...
cms = Sketch.CountMinSketch.new(0.0001, 0.001)
```
...
tk = Sketch.TopK.new(100, epsilon: 0.0001, delta: 0.001)
```
